...
final class BoxHandler
{
    ...
    public function list(): ResponseInterface
    {
        ...
        return $this->responseFactory->json(200, array_map(fn(Box $b) => $b->toArray(), $boxes));
    }
    ...
    public function get(int $id): ResponseInterface
    {
        ...
        return $this->responseFactory->json(200, $box->toArray());
    }

    public function create(RequestInterface $request): ResponseInterface
    {
        $dto = $this->parseAndValidateBox($request);
        $created = $this->boxService->create($dto->width, $dto->height, $dto->length, $dto->maxWeight);

        return $this->responseFactory->json(201, $created->toArray());
    }

    public function update(int $id, RequestInterface $request): ResponseInterface
    {
        $dto = $this->parseAndValidateBox($request);
        $updated = $this->boxService->update($id, $dto->width, $dto->height, $dto->length, $dto->maxWeight);

        if ($updated === null) {
            return $this->responseFactory->json(404, ['error' => 'not_found', 'message' => 'Box not found.']);
        }

        return $this->responseFactory->json(200, $updated->toArray());
    }
